// Alur data: Budget (luar fetch) → getBudgets → localStorage → tampilan

import { useState } from 'react';
import { useFetch } from '../hooks/useFetch';
import { getBudgets, putBudget } from '../API/getData';
...
export default function Budget() {
  // useState → state form + edit
  const [showForm, setShowForm] = useState(false);
  const [editCategory, setEditCategory] = useState(null);
  const [form, setForm] = useState({ category: categories[0].name, limit: '' });

  // useFetch → getData → localStorage
  const { data: budgets, loading, error, refetch } = useFetch(getBudgets);
  ...
  const bdgs = budgets ?? [];
  const totalBudget = bdgs.reduce((s, b) => s + b.limit, 0);
  const totalSpent  = bdgs.reduce((s, b) => s + b.spent, 0);

  // ternary → status budget
  const overBudget = bdgs.filter(b => b.spent > b.limit);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ternary → tampilkan warning hanya jika ada budget terlampaui */}
      {overBudget.length > 0 && (
      ...
      <div className="space-y-3">
        {bdgs.map((b) => {
          ...
          // ternary → warna bar & badge status
          ...
          return (
            <Card key={b.category} padding="p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{formatRupiah(b.spent)} dari {formatRupiah(b.limit)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              {/* ternary → pesan kelebihan */}
              {isOver && (
                <p className="text-xs text-red-500 mt-2">Kelebihan {formatRupiah(b.spent - b.limit)} dari anggaran</p>
              )}
            </Card>
          );
        })}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl p-6"
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-bold text-slate-900">{editCategory ? 'Edit Anggaran' : 'Tambah Anggaran'}</h2>
              <button onClick={() => setShowForm(false)} className="text-slate-400 text-xl">×</button>
            </div>
            <form onSubmit={handleSave} className="space-y-4">
              {/* ternary → tampilkan select hanya saat tambah baru */}
              {editCategory ? (
              ...
              <div>
                ...
                <input type="text" value={form.limit} onChange={e => setForm({ ...form, limit: e.target.value })}
